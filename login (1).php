...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Mempersiapkan pernyataan SQL untuk memeriksa id_admin
    $id_admin_query = "SELECT * FROM tab_admin WHERE id_admin = ?";
    $id_admin_stmt = mysqli_prepare($conn, $id_admin_query);
    mysqli_stmt_bind_param($id_admin_stmt, "s", $id_user);
    mysqli_stmt_execute($id_admin_stmt);
    $id_admin_result = mysqli_stmt_get_result($id_admin_stmt);
    ...
    if (mysqli_num_rows($id_user_result) === 0 && mysqli_num_rows($id_admin_result) === 0) {
        ...
        if (mysqli_num_rows($id_user_result) > 0) {
            ...
            if ($password !== $row["password"]) {
                ...
                // User login successful logic
                $_SESSION["id_user"] = $row["id_user"];
                $_SESSION["nama_lengkap"] = $row["nama_lengkap"];
                $_SESSION["prodi"] = $row["prodi"];
                $_SESSION["semester"] = $row["semester"];
                $_SESSION["id_ukm"] = $row["id_ukm"];
        ...
        // Check admin login
        if (mysqli_num_rows($id_admin_result) > 0) {
            $admin_row = mysqli_fetch_assoc($id_admin_result);
            if ($password !== $admin_row["password"]) {
                $password_error = "Password salah.";
            } else {
                // Admin login successful logic
                $_SESSION["id_admin"] = $admin_row["id_admin"];
                $_SESSION["nama_lengkap"] = $admin_row["nama_lengkap"];
                $_SESSION["level"] = $admin_row["level"];
                $_SESSION["id_ukm"] = $admin_row["id_ukm"];
    ...
    // Menutup pernyataan untuk memeriksa ID admin
    mysqli_stmt_close($id_admin_stmt);
