...
$query = "SELECT nama_ukm FROM tab_ukm";
$result = mysqli_query($conn, $query);

// Initialize an array to store the names of UKMs
$nama_ukms = array();

if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        // Add each UKM name to the array
        $nama_ukms[] = $row["nama_ukm"];
    }
}
...
<nav class="navbar navbar-expand-md navbar-dark fixed-top">
		<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#collapsibleNavbar">
		  ...
		  <!-- Display the UKM names -->
		  <div class="jumbotron">
        <h3>Daftar Nama Unit Kegiatan Mahasiswa</h3>
        <ul class="left-align">
            <?php foreach ($nama_ukms as $nama_ukm) : ?>
                <li><?= $nama_ukm ?></li>
            <?php endforeach; ?>
        </ul>
    </div>
